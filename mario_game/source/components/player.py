...
class Player(pg.sprite.Sprite):
    ...
    def restart(self):
        """MODIFIED: Restart in state WALK"""
        if self.dead:
            self.dead = False
            self.big = False
            self.fire = False
        ...
        self.jump_vel = 0 
        self.y_vel = 0
        self.x_vel = self.player_data[c.PLAYER_SPEED][c.MAX_WALK_SPEED]
    ...
    def setup_speed(self):
        ...
        self.x_vel = speed[c.MAX_WALK_SPEED]
        self.y_vel = 0
    ...
    def walking(self, keys, fire_group):
        ...
        elif True: # keys[tools.keybinding['right']]:
            self.facing_right = True
            if self.x_vel < 0:
                self.frame_index = 5
                self.x_accel = c.SMALL_TURNAROUND
